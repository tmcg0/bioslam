...
// unit test of AngleBetweenAxisAndSegmentFactor
// unit test framework:
...
int test_derivative_numerically(const bioslam::AngleBetweenAxisAndSegmentFactor& fac, const gtsam::Unit3& axis, const gtsam::Point3& v1, const gtsam::Point3& v2);
...
int test_givenPriorsOn3Variables(uint numTests, double errorTol, const gtsam::SharedNoiseModel& factorNoiseModel,
                                 const gtsam::SharedNoiseModel& axisPriorModel, const gtsam::SharedNoiseModel& v1PriorModel,const gtsam::SharedNoiseModel& v2PriorModel, double ang=M_PI/2);
...
int main(){
    ...
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,looseAxis,looseVec,looseVec,ang);
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,looseAxis,tightVec,tightVec,ang);
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,tightAxis,looseVec,tightVec,ang);
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,tightAxis,tightVec,looseVec,ang);
    // also run when ang = 0 and 180 as edge cases
    ang=0.0;
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,looseAxis,looseVec,looseVec,ang);
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,looseAxis,tightVec,tightVec,ang);
    test_givenPriorsOn3Variables(50,0.001,myNoiseModel,tightAxis,looseVec,tightVec,ang);
...
int test_givenPriorsOn3Variables(uint numTests, double errorTol, const gtsam::SharedNoiseModel& factorNoiseModel,
                                 const gtsam::SharedNoiseModel& axisPriorModel, const gtsam::SharedNoiseModel& v1PriorModel,const gtsam::SharedNoiseModel& v2PriorModel, double ang){
    ...
    for(uint i=0;i<numTests;i++){
        ...
        bioslam::AngleBetweenAxisAndSegmentFactor testFac=bioslam::AngleBetweenAxisAndSegmentFactor(axisKey, v1Key, v2Key, ang, factorNoiseModel);
        mygraph.add(testFac);
        ...
        uint nIterations=0;
        double initialAngle=mathutils::unsignedAngle(axis.unitVector(), (v1 - v2));
        ...
        if(finalAngleError>errorTol){ //
            std::cerr<<"error: angle error is greater than "<<errorTol<<"! ("<<finalAngle<<" radians)"<<std::endl<<std::endl;
            return 1;
        }
        // now clear the graph
        myVals.clear();
...
int random_factor_tests(uint nTests){
    ...
    for(uint i=0;i<nTests;i++) {
        ...
        bioslam::AngleBetweenAxisAndSegmentFactor testFac=bioslam::AngleBetweenAxisAndSegmentFactor(axisKey, v1Key, v2Key, ang, myNoiseModel);
...
int test_derivative_numerically(const bioslam::AngleBetweenAxisAndSegmentFactor& fac,const gtsam::Unit3& axis, const gtsam::Point3& v1, const gtsam::Point3& v2){
    ...
    gtsam::Matrix numericalH1=gtsam::numericalDerivative31<gtsam::Vector,gtsam::Unit3,gtsam::Point3,gtsam::Point3>(
            std::function<gtsam::Vector(const gtsam::Unit3&, const gtsam::Point3&, const gtsam::Point3&)>
                    (std::bind(&bioslam::AngleBetweenAxisAndSegmentFactor::evaluateError,fac,_1,_2,_3,boost::none,boost::none,boost::none)),axis,v1,v2,1e-5);
